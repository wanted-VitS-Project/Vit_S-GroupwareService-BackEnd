...
**최종 업데이트**: 2026-08-16 (§0 TL;DR 신설 · §4 이강욱 행 갱신 — 결재 「확인 요구」(`DEL-016`) 는 **2026-08-12 에 구현·확정**됐다(더 이상 「확인 필요」가 아니다) · §2-1 각주 — `tax_invoice_confirm` 은 정산 재설계(`V20260809130000`)에서 **DROP 된 테이블**이라 D-2 목록 7종은 그대로가 맞고, [`BLOCK.md`](BLOCK.md) §8-1 도 **같은 날 갱신돼 이제 일치한다**(상호참조 시제 정정))
**최종 업데이트**: 2026-08-12 (§2-2 — D-3 예외 **2→4건**: 스텝 삭제 시 `step_permission`, 프로젝트 삭제 시 `project_member`·`project_business_category`·스테이지·`stage_permission_default` 정리를 구현하고 근거를 명시. 공통 기준은 **「상위가 복구되지 않는다」**. §3-2 `project` 행 갱신 — 계층 전파는 여전히 없다)
...
- **이 문서가 정하는 것**: 사용자가 **지금** 무언가를 지울 때의 전 도메인 공통 규칙 — soft/hard 판정(D-1·D-2), 하드가 실제로 실행되는 순간(D-3), 참조 대상이 삭제됐을 때 조회가 뭘 내려주나(D-6).
- ⚠️ **조용히 깨지는 함정 ①**: 상세보다 `block` 을 먼저 `save()` 하면 `block.deleted_at` 이 **유실**된다 — 컴파일도 되고 예외도 안 난다 (§3-1).
- ⚠️ **조용히 깨지는 함정 ②**: `BlockDetailPort` 어댑터 미등록은 `registry.find()` 가 **로그도 예외도 없이 넘어간다** — D-4 미러가 거짓이 된다 (§6-3).

| 섹션 | 내용 |
|---|---|
| §1 | 판정은 **질문 3개**로 끝난다 — soft/hard 인가 · 상위 삭제를 어떻게 아나 · 참조가 죽으면 화면에 뭘 띄우나 |
| §2 | 규칙 `D-1`~`D-7`. 실물은 전부 soft, 하드 DELETE 는 **연결 행 7종**뿐이고 트리거는 「사용자의 연결 해제」다 (예외 4건 = §2-2) |
| §3 | 삭제 한 건은 **DDL · Command · 어댑터 · Query 4개 레이어**에 흔적을 남긴다. 전파 진입점은 `step`·`block` 2곳뿐 |
| §4 | 도메인별 현황. `employee` 참조 7곳의 표시 규약(§4-1 — 필드명 `deleted` · 이름 유지 · `resigned` 와 별개)이 전 도메인 정본 |
| §5 | 조회 패턴 A~D 중 **D 만 정답** — 값은 유지하고 `xxxDeleted` 플래그를 함께 내린다 |
| §6 | 함정 — `UNIQUE(…, deleted_at)` 는 무효 · 삭제 순서 · 어댑터 미등록 · 휴지통은 `file`·`image` 2곳뿐 |
| §7·§8 | 판정 예시 · 관련 문서 |
...
> 📌 **`tax_invoice_confirm` 이 왜 이 표에 없나** (2026-08-16 교차 확인 · **[`BLOCK.md`](BLOCK.md) §8-1 과 일치 — 2026-08-16 정합**).
> **정산 재설계 마이그레이션 `V20260809130000` 에서 `payment`·`block_payment_confirm`·`tax_invoice`·`tax_invoice_confirm` 4개가 통째로 `DROP`** 되고 `settlement_block`·`cash_flow`·`tax_invoice` 로 교체됐다.
> **지금 스키마에 없는 테이블이라 D-2 대상이 아니다** — 위 7종 목록은 그대로가 맞다.
> `BLOCK.md` §8-1 은 옛 판에서 이 테이블을 하드 대상으로, `block_payment_confirm` 을 soft 목록에 올려두고 있었으나 **같은 날 갱신돼 현행 정산 블록 상세 `settlement_block`(soft) 기준으로 정리됐다.** 두 문서는 이제 어긋나지 않는다.
>
...
#### ⭐ D-3 의 예외 4건 — 지우는 게 맞다 (2026-08-11 실측 · 2026-08-12 2건 추가)

⚠️ **아래 4건은 D-3 문자 그대로 읽으면 위반처럼 보이지만 올바른 코드다. 지우지 마라.**
...
| ⭐ **`StepCommandService.deleteStep`** — `stepPermissionRepository.deleteByStepId()` (2026-08-12) | 스텝 삭제 시 `step_permission` 을 하드 DELETE | 위 스테이지 건과 **완전히 같은 구조**다 — ① 스텝은 복구가 없다(§6-5) ② 권한 판정은 살아있는 스텝만 조회한다(INV-01) ③ `step.deleted_at` 이 조회를 이미 거른다. 셋 다 「아니오」 |
| ⭐ **`ProjectCommandService.deleteProject`** — `project_member` · `project_business_category` · `stage_permission_default` 동반 삭제 + 스테이지 논리 삭제 (2026-08-12) | 프로젝트 삭제 시 **스텝과 무관하게 남는 것**을 정리 | 프로젝트도 복구가 없다 → 재초대·재연결 대상이 영원히 없다. ⚠️ **계층 전파가 아니다** — 스텝 0개 게이트는 그대로고(§3-2), 스텝·블록·이슈는 애초에 0건이라 손대지 않는다 |

> 🔑 **네 건의 공통 기준은 「상위가 복구되지 않는다」이다.** 상위가 살아 돌아올 수 있으면 D-3 대로 남긴다.
> 블록이 soft 삭제될 때 `issue_block`·`block_file` 을 지우면 안 되는 이유가 그것이다.
...
| 상위 삭제 | 전파 |
|---|---|
| `project` | 스텝 0개일 때만 삭제 허용 → **계층 전파는 없다.** 단 스텝과 무관한 것은 정리한다 — 참여자 · 사업분류 연결 · 스테이지 · 스테이지 권한 기본값 (2026-08-12 · §2-2 예외) |
...
| 대상 | 상태 | 할 일 |
|---|---|---|
| `BID_NOTICE` 상세 어댑터 | ⬜ **미구현 (전파 대상 없음)** | `bid_notice_block` 테이블 자체가 마이그레이션에 **아직 없다** (`BLOCK.md` §4-9, 2026-08-16 재확인) — 어댑터가 없는 게 아니라 상세가 없다. 테이블이 생기면 그때 어댑터를 등록하고 이 행을 D-4 대상으로 올린다. ⚠️ `registry.find(type).ifPresent(...)` 라 어댑터 누락은 **예외도 로그도 없이 조용히 넘어간다** — 테이블 생성 시 반드시 함께 등록할 것 |
...
| 대상 | 상태 | 할 일 |
|---|---|---|
| 진행 중 결재의 블록 삭제 | ✅ **확인 요구 구현·확정** (2026-08-12 · `DEL-016`) | 없음 — 차단 제거(2026-08-10) 이후의 결론이 이것이다. **블록 직접 삭제**는 상신 이후(`IN_PROGRESS`·`REJECTED`·`COMPLETED`)면 409 `APPROVAL_DELETE_CONFIRM_REQUIRED` 로 되묻고, `confirmApprovalCancel=true` 재요청이면 삭제한다. ⛔ **스텝 삭제 cascade 는 확인 없이** 상태 무관 삭제(`DEL-017`) — 결재 때문에 스텝·프로젝트 삭제가 롤백되지 않는다. 상세: [`BLOCK.md`](BLOCK.md) §8 |
