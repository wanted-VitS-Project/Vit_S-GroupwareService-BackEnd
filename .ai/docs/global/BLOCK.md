...
| 템플릿 ⚠️ | **2026-08-13 정정.** 담김: `DRAFT` 상신의 **제목·본문만** / 안 담김: ⛔ **결재선(사람)** · 진행 상태 · 대상 지목. ~~담김: 결재선~~ 은 틀렸다 — 결재자는 `project_member` 여야 하는데 복제는 참여자를 안 옮긴다 → **§9-1** |
| 담당 | 이강욱 |
...
### 9-1. 🚨 결재선은 복사할 수 없다 (2026-08-13 정정)

이 절은 원래 *"결재선이 템플릿의 진짜 값어치"* 라고 적고 있었다. **틀렸다.**

`ApprovalLineEligibilityPolicy` 가 **결재자에게 그 프로젝트의 `project_member` 자격을 요구**한다 (`MASTER` 만 면제).
그런데 프로젝트 복제(`PRJ-018`)는 **참여자를 복사하지 않는다** — 복제본의 사람은 사용자가 새로 지정한다.
두 규칙은 양립하지 않는다.

⚠️ **그대로 복사하면 조용히 깨진다.** 복제 시점에는 정책을 안 타므로 **INSERT 가 성공**하고,
몇 주 뒤 그 결재를 상신하거나 결재선을 고칠 때 `APPROVAL_LINE_APPROVER_NOT_MEMBER` 로 터진다.
그래서 **`approval_line` 은 복사 대상에서 뺀다.** 담기는 것은 `DRAFT` 상신의 제목·본문뿐이며,
"○○ 착수 승인 요청" 같은 **양식 문구**는 이것으로 살아남는다.

> 결재선을 살리고 싶으면 **참여자 복사를 먼저 열어야 한다.** 순서가 반대면 부정합 행만 쌓인다.

### 9-2. 저장소 — `template` 테이블 없이 성립한다

> ⭐ **근거 정정 (2026-08-13).** 이 절은 *"확정 ERD 에 `template` 테이블이 없어 저장소가 정해진 뒤에야 유효하다"* 고 적고 있었다.
> **살아있는 프로젝트를 원본으로 삼는 복제**(`PRJ-018` — [`../domain/프로젝트/PRJ-V1-API.md`](../domain/프로젝트/PRJ-V1-API.md) §4-J)는 **신규 테이블 0건**으로 성립한다.
> [`../domain/HANDOFF.md`](../domain/HANDOFF.md) T1 에 남는 것은 **원본과 무관하게 독립 존재하는 사용자 정의 템플릿** 뿐이다.

| 단계 | 담기는 것 | 상태 |
|------|-----------|------|
| **v1** | 블록 **껍데기만** (`type`·`title`·배치). 상세는 기존 생성 3단계대로 **빈 행** | `📝 초안` — ⛔ 구현 금지 |
| **v2** | `TEXT` 본문 · `CHECKLIST` 항목 · `APPROVAL` 상신 제목·본문 | v1 확정·구현 후 착수 |

⭐ **v2 확장 방식** — `BlockDetailPort` 에 `default Long copyDetail(fromTypeId, toBlockId) { return createDetail(toBlockId); }` 를 추가한다.
구현하지 않은 타입은 지금과 똑같이 빈 행이 되므로, **타입 담당자가 각자 자기 어댑터에서 순차로 열면 된다** (§2-1 소유 경계 그대로 · Block 도메인 파일은 안 고친다).
