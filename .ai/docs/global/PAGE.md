# 🖥️ 페이지 체계 — 탭 8개 · 화면 36개

**최종 업데이트**
- 2026-08-03 — 결재관리 탭(P-31) 설명 실제 화면 기준으로 수정, 결재 상태도 회차 기반으로 갱신, 반려 사유 필수→선택 정정
- 2026-08-01 — 최초 작성
**작성/관리**: 동훈

> 이 문서는 **어떤 화면이 있고 각 화면이 무슨 역할인가**, 그리고 **누가 만드나**를 정의한다.

> 🔑 **권한 표기** — 전역 role 은 `ADMIN` > `MASTER` > `MEMBER` 3종이고 **서열이 이름과 반대다.**
> `BIDDING` · `FINANCE` 는 role 이 아니라 **`page_permission.page_code`** 값이다. **`page_code` 는 이 2개뿐이다.**
...
| 규칙 | 확정 |
|------|------|
| **`page_code` 는 2개뿐** | `BIDDING` · `FINANCE`. 늘리지 마라 |
...
| ID | 화면 | 권한 | **역할** | 백엔드 | 프론트 |
|----|------|------|---------|-------|-------|
| **P-31** | 결재관리 탭 | 전원 | `결재 요청받음 / 내가 올린 결재 / 전체(master만)` 3탭. **내 결재선 건만.** 프로젝트를 안 열고도 결재할 수 있게 하는 게 존재 이유다 | 강욱 | 민지 |
| **P-30** | 결재 처리 (승인 / 반려 + 사유) | 전원 | **P-31 안에서** 실제 판정을 내린다. 반려 사유는 선택 입력 | 강욱 | 민지 |
...
| **P-61** | 사원 · 부서 관리 | `ADMIN` | 사원 등록·부서 배정·퇴사 처리. **퇴사자 담당 항목 경고 목록**을 여기서 뽑는다 | 동현 | 민지 |
...
| **동현** | 백엔드 | auth P-00·P-01 · 관리자 P-60·P-61 · 문서 P-24 |
...
**정산 메뉴에서 등록·수정은 할 수 없다.** 등록은 재무 탭(P-40), 회차 생성은 입금확인 블록이다.
...
프로젝트 > 스텝 > 결재 블록(P-29)  →  상신 (대상 파일 블록 지목 + 버전 고정)
                                          ↓  알림(ACTION)
결재관리 탭(P-31)                  →  결재 요청받음 / 내가 올린 결재 / 전체  →  P-30 승인 · 반려(사유 선택)
```
...
stateDiagram-v2
    [*] --> DRAFT: 결재 블록 생성
    DRAFT --> IN_PROGRESS: 상신(최초/재상신 동일 로직) — 문서·결재선 확정
    IN_PROGRESS --> IN_PROGRESS: 순차 승인 (다음 노드 활성)
    IN_PROGRESS --> REJECTED: 반려 + 사유(선택)
    REJECTED --> DRAFT: 재상신 준비 — 새 회차(revision) 생성, 반려 지점부터 결재선 재구성
    IN_PROGRESS --> COMPLETED: 결재선 마지막 순번 승인
    COMPLETED --> [*]: 완료. 되돌리려면 새 결재
```

**재개 지점은 항상 반려자부터다.** 상신마다 `approval_revision`(회차)이 새로 생성되어 독립적으로 이력이 남으며, 파일 버전 변경 여부와 무관하게 반려 지점부터 재개한다 — 회차 개념이 없던 구버전 스키마의 "버전 바뀌면 전원 재승인" 규칙은 폐기됐다.
**최종 결재자는 `MASTER` 가 아니어도 된다** — 결재선 마지막 순번인 사람이 승인하면 완료된다 (`BLOCK.md` §4-8, `PERMISSION.md` §7-2).

> 🔖 회수(WITHDRAWN, 상신 후 첫 결재자 승인 전 자진 철회) 기능은 v1 범위에서 만들지 않는다. 필요해지면 백로그에서 검토한다.
...
flowchart TD
    subgraph PRJ["프로젝트 — 실무자"]
        S1[정산 N차 스텝<br/>+ 입금확인 블록 1개] --> S2[청구서 파일 블록]
        S2 --> S3[P-29 결재]
    end
...
| **P-70 · P-64** | 🚨 `template` 테이블이 확정 ERD 에 없다 |
