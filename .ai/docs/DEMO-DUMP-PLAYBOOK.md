...
## TL;DR (§0 은 아래 「0. 붙여넣어 쓰는 프롬프트」 절이다)
...
| §2 | 파일 분할과 실행 순서 — **`01~07 → 09 → 10~23(후속 패치) → 08_activity_log`**, 활동 로그는 전체 시드의 맨 마지막 |
...
**08을 마지막에 두는 이유**: `activity_log` 에는 `deleted_at` 이 없고 `block_id` 는 `NOT NULL` 이다. 되돌릴 수단이 없으니 데이터가 다 들어간 뒤 한 번만 실행한다.

⚠️ **실제 실행 순서는 `01~07 → 09 → 10~23 → 08` 이다** (2026-08-16 정정). 후속 패치 `10_`~`23_` 이 블록·파일·프로젝트를 더 만들므로, 08 은 **번호상 8번째가 아니라 전체 시드의 맨 마지막**에 돌린다. `src/main/resources/db/demo/README.md` 와 같은 순서다.
