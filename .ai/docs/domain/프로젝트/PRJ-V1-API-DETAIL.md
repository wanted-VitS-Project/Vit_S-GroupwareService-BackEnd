...
**최종 업데이트**: 2026-08-01 (신규 ERD 정합 반영)
...
> 모든 응답은 `httpStatus` · `message` · `data` 봉투를 쓴다. 아래 Response Parameter 의 들여쓴 항목은 `data` 하위다.
> 공통 에러(`401 AUTH_TOKEN_EXPIRED` · `403 PROJECT_ACCESS_DENIED` · `404 PROJECT_NOT_FOUND`)는 각 Status Code 표에 반복 기재한다.
...
| ⛔ `block.type_id` | **폐기 (2026-08-03).** 응답에 `typeId` 를 내리지 않는다 → [`ERD.md`](ERD.md) §0-9 |
| ⛔ `step.step_type` | **폐기 (2026-08-03).** 송부 스텝 개념이 없다 (`PRJ-V1.md` STP-007) |
| 블록 삭제 잠금 | **4종** — 입금 연결 입금확인 · 계산서 연결 조회 · 진행 중 결재 · 결재 대상 파일 (`BLOCK.md` §8) |

⚠️ **ERD Cloud 미반영분이 있다** → [`ERD.md`](ERD.md) §8. 코드는 [`ERD.md`](ERD.md) 를 따른다.

---

# POST `/api/v1/projects` — 프로젝트 직접 생성
...
생성 시 상태는 시스템이 `NOT_STARTED` 로 설정하고, `bid_notice_id` 는 `NULL` 이다 (PRJ-001).
...
## Success Example
```
{ "httpStatus":200, "message":"사업 카테고리 해제 성공", "data":null }
```
...
## Success Example
```
{ "httpStatus":200, "message":"참여자 제거 성공", "data":null }
```
...
| `blocks[].type` | String | 블록 타입 (ERD enum **9값** · ⛔ `MEMO` 폐기) |
...
⛔ **`typeId` 를 내리지 않는다.** `block.type_id` 는 폐기됐다 ([`ERD.md`](ERD.md) §0-9).
...
**`type` 허용값 — ✅ 9종 확정 (2026-08-03 · `MEMO` 폐기)**

| 값 | 상세 테이블 | 카디널리티 | 담당 |
| --- | --- | --- | --- |
| `TEXT` | `text` | 1:1 | 정림 |
| `IMAGE` | `img_block` → `image` | 1:N | 정림 |
| `CHECKLIST` | `chk_block` → `checklist` | 1:N | 정림 |
| `FILE` | `block_file` | 1:N | 김동현 |
| `PAYMENT_CONFIRM` | `block_payment_confirm` | 1:1 | 동훈 |
| `TAX_INVOICE_VIEW` | `tax_invoice_confirm` | 1:1 | 동훈 |
| `PERFORMANCE_VIEW` | **없음** 🚨 T2 미결 | — | 동훈 |
| `APPROVAL` | `approval` | 1:1 | 이강욱 |
| `AI` | `vitamate_block` | 1:1 | 정현 |

> ⛔ **`MEMO` 는 폐기됐다 (2026-08-03).** enum 에 없고, **FE 타입 선택 목록에서도 빼라.**
> 자유 서술은 `TEXT` 가 담당한다 ([`BLOCK.md`](../../global/BLOCK.md) §4-1 *"본문·목차·회의 메모"*).
> ✅ 이제 [`BLOCK.md`](../../global/BLOCK.md) 9종 카탈로그와 [`ERD.md`](ERD.md) enum 이 일치한다 (`PRJ-V1-API.md` §4-B).

블록은 **항상 스텝에 붙는다.** `block.project_id` 는 시스템이 비정규화로 함께 채운다 (BLK-002).
⛔ `block.type_id` 는 **없다** (2026-08-03 폐기). 상세 행은 `{상세테이블}.block_id` 로 블록을 가리킨다 — 응답에 `typeId` 를 내리지 않는다.
...
| 400 | Bad Request | `BLOCK_TYPE_INVALID` | 정의된 **9종** 밖의 타입 (`MEMO` 포함) |
...
## Success Example
```
{ "httpStatus":200, "message":"블록 삭제 성공", "data":null }
```
