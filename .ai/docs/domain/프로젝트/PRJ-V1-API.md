...
**최종 업데이트**: 2026-08-03 (✅ 사번 `VARCHAR(20)` 확정 · ✅ **`MEMO` 폐기 → 블록 9종** · 잠금 4종 · TXL-001B 반영)
**최종 업데이트**: 2026-08-03 (`step_type` 폐기 · 발주처·계약금액 해소 · `block.owner` 신설 · [`ERD.md`](ERD.md) 확정)
**최종 업데이트**: 2026-08-01 (신규 ERD 정합 반영 — 사번 `VARCHAR(20)` · status enum · 블록 타입 enum)
**담당**: 동훈
...
> 이 문서는 요구사항(`PRJ-V1.md`)에서 도출한 **설계안**이다. 노션에 올려 `✅ 확정` 을 받아야 코드를 쓴다.
...
| | `started_on` / `ended_on` | DATE | `startedOn` / `endedOn` | LocalDate |
| `block` | `type` | ENUM **9값** | `type` | String |
...
> ⛔ **`block.type_id` 도 폐기됐다 (2026-08-03).** 블록 응답에 `typeId` 를 내리지 않는다 → [`ERD.md`](ERD.md) §0-9
> ✅ **`activity_log` 는 스키마 확정** → [`ERD.md`](ERD.md) §5-4. `project_id` 는 **NULL 허용**이다 (미매칭 입금 로그). ⚠️ ERD Cloud 미반영
...
| `block.type` (**9종**) | `TEXT` · `IMAGE` · `FILE` · `CHECKLIST` · `PAYMENT_CONFIRM` · `TAX_INVOICE_VIEW` · `PERFORMANCE_VIEW` · `APPROVAL` · `AI` — ⛔ `MEMO` 폐기 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 개발 전 | 📝 초안 | Project | 프로젝트 목록 조회 | GET | `/api/v1/projects` | 참여자 | 상태·카테고리·기간으로 필터한 프로젝트 목록을 조회한다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 상세 조회 | GET | `/api/v1/projects/{projectId}` | 참여자 | 프로젝트 기본 정보·진척률·카테고리를 조회한다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 직접 생성 | POST | `/api/v1/projects` | 전체 사용자 | 공고 없이 프로젝트를 생성한다. 상태는 `NOT_STARTED`. |
| 개발 전 | 📝 초안 | Project | 프로젝트 수정 | PATCH | `/api/v1/projects/{projectId}` | 프로젝트 EDITOR | 과업명·설명·기간·**발주처**·계약금액을 수정한다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 상태 변경 | PATCH | `/api/v1/projects/{projectId}/status` | 프로젝트 EDITOR | `NOT_STARTED` → `IN_PROGRESS` → `SETTLEMENT` → `COMPLETED` 로 상태를 바꾼다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 종결 | POST | `/api/v1/projects/{projectId}/close` | 프로젝트 EDITOR | 사유를 붙여 프로젝트를 `CLOSED` 로 만든다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 삭제 | DELETE | `/api/v1/projects/{projectId}` | 프로젝트 EDITOR | 진행 전 · 스텝/블록 0개일 때만 논리 삭제한다. |
| 개발 전 | 📝 초안 | Project | 프로젝트 진척률 조회 | GET | `/api/v1/projects/{projectId}/progress` | 참여자 | 완료 스텝 / 전체 스텝 진척률을 조회한다. |
| 개발 전 | 📝 초안 | Project | 사업 카테고리 연결 | POST | `/api/v1/projects/{projectId}/business-categories` | 프로젝트 EDITOR | 사업 카테고리를 복수 연결한다. |
| 개발 전 | 📝 초안 | Project | 사업 카테고리 해제 | DELETE | `/api/v1/projects/{projectId}/business-categories/{categoryId}` | 프로젝트 EDITOR | 연결된 사업 카테고리를 해제한다. |
| 개발 전 | 📝 초안 | Member | 참여자 목록 조회 | GET | `/api/v1/projects/{projectId}/members` | 참여자 | 참여자와 `permission`·퇴사 여부를 조회한다. |
| 개발 전 | 📝 초안 | Member | 참여자 추가 | POST | `/api/v1/projects/{projectId}/members` | 프로젝트 EDITOR | 참여자를 **한 명씩** 추가한다. |
| 개발 전 | 📝 초안 | Member | 참여자 권한 변경 | PATCH | `/api/v1/projects/{projectId}/members/{memberId}` | 프로젝트 EDITOR | 참여자 권한을 `VIEWER`/`EDITOR`/`NONE` 으로 바꾼다. |
| 개발 전 | 📝 초안 | Member | 참여자 제거 | DELETE | `/api/v1/projects/{projectId}/members/{memberId}` | 프로젝트 EDITOR | 참여자를 프로젝트에서 제거한다. |
| 개발 전 | 📝 초안 | Stage | 스테이지 목록 조회 | GET | `/api/v1/projects/{projectId}/stages` | 참여자 | 스테이지를 순서대로 조회한다. |
| 개발 전 | 📝 초안 | Stage | 스테이지 생성 | POST | `/api/v1/projects/{projectId}/stages` | 프로젝트 EDITOR | 스테이지를 생성한다. |
| 개발 전 | 📝 초안 | Stage | 스테이지 수정 | PATCH | `/api/v1/stages/{stageId}` | 프로젝트 EDITOR | 스테이지 이름을 수정한다. |
| 개발 전 | 📝 초안 | Stage | 스테이지 순서 변경 | PATCH | `/api/v1/projects/{projectId}/stages/order` | 프로젝트 EDITOR | 스테이지 순서를 일괄 재정렬한다. |
| 개발 전 | 📝 초안 | Stage | 스테이지 삭제 | DELETE | `/api/v1/stages/{stageId}` | 프로젝트 EDITOR | 하위 스텝 이전 대상을 지정해 스테이지를 삭제한다. |
| 개발 전 | 📝 초안 | Stage | 하위 스텝 권한 일괄 적용 | POST | `/api/v1/stages/{stageId}/step-permissions` | 프로젝트 EDITOR | 스테이지 하위 스텝에 권한을 일괄 적용한다. |
| 개발 전 | 📝 초안 | Step | 스텝 목록 조회 | GET | `/api/v1/projects/{projectId}/steps` | 참여자 | 스텝을 스테이지·순서 기준으로 조회한다. |
| 개발 전 | 📝 초안 | Step | 스텝 상세 조회 | GET | `/api/v1/steps/{stepId}` | 스텝 접근 권한 | 스텝 속성·상태·진척률을 조회한다. |
| 개발 전 | 📝 초안 | Step | 스텝 생성 | POST | `/api/v1/projects/{projectId}/steps` | 프로젝트 EDITOR | 스텝을 생성한다. `stage_id` 는 `NULL` 허용, `project_id` 는 항상 채운다. |
| 개발 전 | 📝 초안 | Step | 스텝 수정 | PATCH | `/api/v1/steps/{stepId}` | 스텝 EDITOR | 이름·기간·책임자·소속 스테이지를 수정한다. |
| 개발 전 | 📝 초안 | Step | 스텝 순서 변경 | PATCH | `/api/v1/projects/{projectId}/steps/order` | 프로젝트 EDITOR | 스텝 순서를 일괄 재정렬한다. |
| 개발 전 | 📝 초안 | Step | 스텝 상태 변경 | PATCH | `/api/v1/steps/{stepId}/status` | 스텝 EDITOR | `NOT_STARTED` / `IN_PROGRESS` 로 바꾼다. `DONE` 은 완료 처리 API 소관. |
| 개발 전 | 📝 초안 | Step | 스텝 완료 처리 | POST | `/api/v1/steps/{stepId}/complete` | 스텝 EDITOR | 미완료 이슈 처리 방식을 지정해 스텝을 완료한다. |
| 개발 전 | 📝 초안 | Step | 스텝 삭제 | DELETE | `/api/v1/steps/{stepId}` | 프로젝트 EDITOR | 이슈 처리 방식을 지정해 스텝을 삭제한다. |
| 개발 전 | 📝 초안 | StepPermission | 스텝 권한 목록 조회 | GET | `/api/v1/steps/{stepId}/permissions` | 프로젝트 EDITOR | 스텝 권한 오버라이드 행을 조회한다. |
| 개발 전 | 📝 초안 | StepPermission | 스텝 권한 부여·변경 | PUT | `/api/v1/steps/{stepId}/permissions/{userId}` | 프로젝트 EDITOR | 스텝별 `VIEWER`/`EDITOR`/`NONE` 을 지정한다. |
| 개발 전 | 📝 초안 | StepPermission | 스텝 권한 회수 | DELETE | `/api/v1/steps/{stepId}/permissions/{userId}` | 프로젝트 EDITOR | 오버라이드 행을 지워 프로젝트 권한 상속으로 되돌린다. |
| 개발 전 | 📝 초안 | Block | 스텝 블록 일괄 조회 | GET | `/api/v1/steps/{stepId}/blocks` | 스텝 접근 권한 | 블록과 타입별 상세를 한 번에 조회한다. |
| 개발 전 | 📝 초안 | Block | 블록 생성 | POST | `/api/v1/steps/{stepId}/blocks` | 스텝 EDITOR | **9종** 타입 안에서 블록을 생성한다. `PAYMENT_CONFIRM`·`TAX_INVOICE_VIEW` 는 **스텝당 1개** 검사 (PCB-001B · TXL-001B). |
| 개발 전 | 📝 초안 | Block | 블록 배치 변경 | PATCH | `/api/v1/steps/{stepId}/blocks/layout` | 스텝 EDITOR | 드래그 결과를 일괄 반영한다. |
| 개발 전 | 📝 초안 | Block | 블록 삭제 | DELETE | `/api/v1/blocks/{blockId}` | 스텝 EDITOR | 잠금 4종을 검사한 뒤 논리 삭제한다. |
| 개발 전 | 📝 초안 | IssueBlock | 이슈-블록 연결 | POST | `/api/v1/blocks/{blockId}/issues` | 스텝 EDITOR | 같은 스텝의 이슈를 블록에 연결한다. |
| 개발 전 | 📝 초안 | IssueBlock | 이슈-블록 연결 해제 | DELETE | `/api/v1/blocks/{blockId}/issues/{issueId}` | 스텝 EDITOR | 이슈-블록 연결을 해제한다. |
| 개발 전 | 📝 초안 | ActivityLog | 활동기록 조회 | GET | `/api/v1/projects/{projectId}/activity-logs` | 참여자 | 프로젝트 활동기록을 최신순으로 조회한다. |
...
### 4-B. 블록 타입 → ✅ **9종 확정 · `MEMO` 폐기 (2026-08-03)**
...
| 출처 | 값 | 판정 |
| --- | --- | --- |
| [`BLOCK.md`](../../global/BLOCK.md) (제목·§3·§4 카탈로그·§5 요약표) | **9종** | ✅ **이제 양쪽이 일치한다** |
| [`ERD.md`](ERD.md) `block.type` | **9종** (`MEMO` 제거) | ✅ |

⚠️ **ERD Cloud 의 `block.type` enum 에서 `MEMO` 를 빼야 한다** → [`ERD.md`](ERD.md) §8-5
...
| 6 | ~~`block.type_id` 운용 방식~~ | ✅ **폐기 확정 (2026-08-03).** 상세는 `{상세테이블}.block_id` 단방향. 블록 응답에서 `typeId` 를 내리지 않는다 |
