...
- ~~형식: `{도메인}_{일련번호}` — 예) `AUTH_001`, `USER_014`~~
  🔴 **이 규칙은 실제 명세와 다르다** (2026-08-04 확인). 노션 확정본은 **의미식**을 쓴다 —
...
프론트는 도메인 코드 분기의 **폴백**으로 이 4개를 처리해야 한다.
...
>
> 🚨 **도메인 에러에 `COMMON_*` 을 쓰지 마라.** 이 4개는 `GlobalExceptionHandler` 만 발급한다.
> 구현 위치: `global/presentation/api/common/GlobalExceptionHandler.java`
>
> 📌 **노션 미반영** — §3 전체가 미확정 상태라 이 표도 팀 합의 후 노션에 올려야 한다.
> 다만 **코드에는 이미 4개가 다 나가고 있으므로**, 문서가 현실을 따라가도록 먼저 적어둔다.
...
- [ ] 🔴 **에러 코드 표기 방식 확정 — 번호식(`AUTH_001`) vs 의미식(`AUTH_LOGIN_FAILED`)**
      실제 노션 확정본은 전부 **의미식**이다. §3-3 의 번호식 규칙이 사문화돼 있다
- [ ] 에러 코드 도메인 접두어 배분
- [ ] 🔴 **`COMMON_*` 4종을 노션에 반영** (§3-3) — 코드에는 이미 나가고 있는데 명세에 없다.
      프론트가 폴백 분기를 못 짠다
