...
**최종 업데이트**: 2026-08-07 (이동 대상 판정 방식 변경 — block 경유 → 발행 도메인이 직접 지정. **이동 대상 조회(#2) 응답 형태는 동일**하고 404 사유만 축소, **목록 조회(#1)에서는 `blockId` 필드 제거**) · 2026-08-06 (개별 삭제 API 폐지 → `RET-001`) · **담당**: 이강욱
