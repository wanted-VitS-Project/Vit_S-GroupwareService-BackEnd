...
| RT-007 | 세션이 끊기면 **서버가 연결을 끊는다** | 로그아웃·세션 만료·계정 잠금(강제 세션 종료) 시 **서버가 최대 15초(하트비트 주기) 안에 그 구독을 종료한다.** 이 프로젝트가 JWT 대신 서버측 세션을 택한 이유가 즉시 무효화라, 열린 스트림도 함께 끊어야 그 전제가 유지된다. 이후 재연결하면 401 이며, **`EventSource` 는 401 을 받아도 재연결을 계속 시도하므로** 프론트가 `onerror` 에서 `es.close()` 하고 로그인 화면으로 보내야 한다 — 안 하면 무한 재시도 루프가 된다 |
