...
| RT-007 | 세션이 끊기면 구독도 끊긴다 | 로그아웃·세션 만료·계정 잠금 후 재연결하면 401 이다. **`EventSource` 는 401 을 받아도 재연결을 계속 시도하므로** 프론트가 `onerror` 에서 `es.close()` 하고 로그인 화면으로 보내야 한다 — 안 하면 무한 재시도 루프가 된다 |
