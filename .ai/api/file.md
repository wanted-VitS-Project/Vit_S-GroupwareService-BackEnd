...
> ⚠️ 이전 **A안**(블록 삭제 시 파일을 활성 상태로 남겨 고아 파일로 둠)은 **폐기**했다(2026-08-16). 그래서 **활성 고아 파일은 더 이상 존재하지 않는다** — 블록이 삭제된 파일은 휴지통(§13)에만 있다. 배선 구현(block→file 일괄 트래시)이 끝나기 전까지는 §12/§13 코드가 아직 A안(활성 고아 반환)일 수 있다.
...
- ⛔ **활성 고아 파일 없음(D안 · 2026-08-16)** — 블록이 삭제되면 그 파일은 휴지통(§13)으로 가므로 이 활성 목록에는 나오지 않는다. 따라서 활성 목록에서 `blockDeleted` 는 항상 `false` 다(`blockDeleted:true` 는 §13 휴지통에서만 나타난다). 이전 "고아 파일 포함" 규칙은 폐기했다.
