...
> - DB 는 `uk_department_parent_name (parent_id, name)` 복합 유니크로 **자식(하위 부서) 동명을 막는다.**
> - MySQL 은 `parent_id` 가 `NULL` 인 행끼리는 UNIQUE 로 안 막으므로, **최상위 동명은 서버가 app 레벨(`parent_id IS NULL` 검사)로 막는다.**
