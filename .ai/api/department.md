...
**최종 업데이트**: 2026-08-06 (§2·§3 부서명 중복을 전체 → 같은 상위 부서(형제) 범위로 완화 · 최상위끼리는 전체 유니크 유지) · **담당**: 김동현 · Domain `인사` · SUB-Domain `Department`
...
⛔ **부서명은 같은 상위 부서 안에서만 중복될 수 없다** (`DEPT-006`, 2026-08-06 변경). `기술본부 > 개발팀` 과 `SI본부 > 개발팀` 처럼 상위가 다르면 같은 이름을 허용한다. **최상위 부서끼리는 전과 같이 전체 유니크**다.

> 🔑 **형제 유니크로 완화 (2026-08-06)** — 서로 다른 본부 아래 같은 이름의 팀이 흔해 전체 유니크가 실제 조직과 안 맞았다. FE 요청으로 완화.
> - DB 는 생성 열 `parent_key = COALESCE(parent_id, 0)` 에 `uk_department_parent_name (parent_key, name)` 복합 유니크를 걸어 **자식과 최상위 동명을 모두 막는다.** (최상위는 `parent_key=0` 을 공유하므로 전체 유니크로 남고, `parent_id` 는 1 부터라 0 과 충돌하지 않는다.)
> - MySQL 은 `parent_id` 가 `NULL` 인 행끼리는 UNIQUE 로 안 막아 최상위 동명이 새므로, NULL 을 0 으로 정규화한 이 생성 열을 쓴다. 서비스의 형제 검사(app 레벨)는 친절한 409 선처리이고, 최종 방어선은 이 DB 제약이다.
> - ⚠️ **FE 대응 필요** — 같은 이름의 팀이 둘 이상 존재할 수 있으므로 **사원 등록 부서 드롭다운은 경로(`기술본부 / 개발팀`)로 구분**해 보여줘야 한다. (프론트 대응 예정)
...
| 409 | `DEPT_NAME_DUPLICATED` | 같은 상위 부서 안에 동명 존재 (최상위끼리는 전체 기준) |
...
⛔ **중복 검사는 자기 자신을 제외하고, 같은 상위 부서(형제) 범위**다 (§2 와 동일 규칙). 상위는 바뀌지 않으므로 그 부서의 현재 `parent_id` 기준 형제끼리 비교한다.
...
| DEPT-006 | 부서 관리 | 부서 생성 | 부서명 중복 차단 | 시스템은 같은 상위 부서 안에 이미 존재하는 부서명의 생성을 차단해야 한다(최상위끼리는 전체 기준). |
