...
> 만료 시각을 화면에 띄우거나 "연장" 버튼을 만들려면 `GET /api/v1/auth/session`(§7)을 쓴다 — 호출 자체가 연장이다.
...
> **검증** — 확장자 화이트리스트(`jpg`·`jpeg`·`png`·`gif`) + 매직바이트 + `ImageIO` 디코딩(디코딩 전 픽셀 수 상한 검사 포함)까지 확인한다(확장자만 바꾼 위장 파일·손상 파일·디코딩 폭탄 차단). **`webp` 는 제외** — JDK 에 내장 디코더가 없어 실제 디코딩·픽셀 검사를 못 해 대형 webp 가 상한을 우회하므로, 안전한 검증기(외부 라이브러리)가 붙기 전까지는 허용하지 않는다.
