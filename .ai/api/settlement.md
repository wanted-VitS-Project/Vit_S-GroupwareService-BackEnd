...
(`V20260812110000__add_version_settlement.sql` — ⚠️ 문서 §7-1 표는 `V20260811150000`을 배정했으나
issue 도메인과 충돌해서 150000 → 170000 → 170100 → 110000/0812까지 재배정됨(develop이 날짜를
넘겨 계속 진행 중이라 CI 마이그레이션 검증에 반복해서 걸림). 문서 수정 권한이 없어 이 사실은
마이그레이션 파일 주석에만 남김 — 김동현님께 별도 전달 필요).
