...
**최종 업데이트**: 2026-08-12 (`APPROVAL_ALREADY_SUBMITTED`(409) 신설 — 상신 이후 블록 직접 삭제 차단. 🔴 프론트 공유 필요)
...
> 🔴 **2026-08-12 — 계약 변경 1건 (프론트 공유 필수).** 위 2026-08-10 의 409 폐기를 **되돌린다.**
>
> | 항목 | 내용 |
> |---|---|
> | 신설 | `APPROVAL_ALREADY_SUBMITTED` (409) — **한 번 상신된 결재의 블록은 직접 삭제할 수 없다.** `approval.status` 가 `IN_PROGRESS`·`REJECTED`·`COMPLETED` 이면 거부한다 |
> | 문구 | **코드는 하나, `message` 는 상태별로 다르다** — `IN_PROGRESS` "진행 중인 결재는 삭제할 수 없습니다." · `REJECTED` "반려된 결재는 삭제할 수 없습니다." · `COMPLETED` "완료된 결재는 삭제할 수 없습니다." 분기는 `code` 로 하고 `message` 는 그대로 노출하면 된다 |
> | 삭제 허용 | `DRAFT`(아직 요청이 안 갔다) · `CANCELED`(이미 종결됐다) |
> | 발생 지점 | `DELETE /api/v1/blocks/{blockId}` — **블록 도메인 엔드포인트다.** 결재 API 12개는 영향 없다 |
> | 이름 | 폐기된 `APPROVAL_IN_PROGRESS` 를 되살리지 않는다. 세 상태를 함께 덮으므로 "진행 중"이 맞지 않는다 |
...
>
> 🔴 **미결 — 블록 삭제 API 명세 파일이 없다.** `.ai/api/` 에 `block.md` 가 없어 이 409 를 블록 엔드포인트
> 명세에 적을 곳이 없다. **어디에 적을지와 응답 본문에 진행 중 결재 정보를 담을지**를 블록 담당자와 정해야 한다
> (`../API.md` §0 — 명세 없는 엔드포인트를 임의로 만들지 않는다).
