...
> `ADMIN`은 인사 전용이므로 결재자 지정 권한이 없다.
...
⚠️ **`ADMIN`의 `scope` 해석 (2026-08-17 신설)** — `ADMIN`은 기안자·결재자가 될 수 없어 `drafted`·`pending`이 **구조적으로 항상 0건**이다. 그래서 `drafted`(및 미지정)는 서버가 `all`로 해석한다. `pending`("내가 처리할 결재")은 승격하지 않고 0건을 그대로 돌려준다 — 탭 이름과 내용이 어긋나지 않게 하기 위함이다.
