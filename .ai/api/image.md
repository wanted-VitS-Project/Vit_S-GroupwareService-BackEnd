...
> ⚠️ `page × size`(건너뛸 행 수)가 **100,000을 넘으면** `400 IMG-012`로 거부한다(2026-08-16 추가) — 프로젝트
...
| `data.images[].stepName` | String | 속한 스텝 이름 (2026-08-14 추가) |
| `data.images[].imgId` | Long | 이미지 ID |
| `data.images[].originalName` | String | 원본 파일명 |
| `data.images[].imageUrl` | String | 저장소 이미지 URL(presigned) |
| `data.images[].caption` | String | 이미지 캡션 |
...
| 400 | Bad Request | `IMG-012` | "페이지 조회 조건이 올바르지 않습니다." — `page`<0 · `size`≤0 또는 >100 · `page`×`size`>100,000 (2026-08-16 추가) |
...
> 🔄 **삭제 후 뒤쪽 이미지 orderIndex 재정렬 (2026-08-17, 프론트 요청)** — 삭제된 이미지보다 orderIndex가 큰
> 활성 이미지 전부를 1씩 당겨서, 삭제 후에도 그 블록의 orderIndex가 빈자리 없이 `1..N`으로 이어지게 한다.
> (예: 1,2,3,4 중 2번 삭제 → 남은 것들이 1,2,3이 됨. 기존엔 1,3,4로 구멍이 남아있었다.) 수정 API(PATCH)는
> 요청 배열의 위치를 그대로 orderIndex로 다시 매겨서 이미 압축돼 있었지만, 이 단건 삭제 API는 삭제 대상
> 하나만 지우다 보니 이 재정렬이 빠져 있었다.
