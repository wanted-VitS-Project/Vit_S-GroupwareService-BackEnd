...
CREATE TABLE `cash_flow` (
    ...
    `type`            ENUM('INCOME','OUTCOME') NULL COMMENT '입출금 구분',
    `traded_at`       DATETIME      NULL     COMMENT '거래일시',
    `amount`          DECIMAL(18,2) NULL     COMMENT '금액',
    `depositor_name`  VARCHAR(250)  NULL     COMMENT '내용 · 입금자명/수취인명',
    `bank_memo`       VARCHAR(500)  NULL     COMMENT '적요/메모',
    `source_type`     ENUM('MANUAL','CSV','API') NULL COMMENT '수집 경로',
...
CREATE TABLE `tax_invoice` (
    ...
    `settle_block_id`  BIGINT         NULL     COMMENT '연결된 정산 블록 아이디 (NULL = 미연결)',
    `type`             ENUM('INCOME','OUTCOME') NULL COMMENT '입출금(매출/매입) 구분',
    `approval_no`      VARCHAR(100)   NOT NULL COMMENT '승인번호',
    `issued_no`        DATE           NULL     COMMENT '발행일',
    `supplier_biz_no`  VARCHAR(20)    NULL     COMMENT '공급자 사업자번호 (외주 구분용)',
    `supply_amount`    DECIMAL(18,2)  NULL     COMMENT '공급가액',
    `tax_amount`       DECIMAL(18,2)  NULL     COMMENT '세액',
    `total_amount`     DECIMAL(18,2)  NULL     COMMENT '총액',
    `buyer_name`       VARCHAR(250)   NULL     COMMENT '공급받는자 상호명',
    ...
    `source_type`      ENUM('CSV','HOMETAX_API') NULL COMMENT '수집 경로',
