...
⛔ **08 은 반드시 마지막**이다 — 번호상 8번째가 아니라 **`01~07 → 09 → 10~23 → 08` 순서로, 후속 패치 전부 뒤에 08 을 돌려라.**
`activity_log` 에는 `deleted_at` 이 없어(`D-5`) 되돌릴 수단이 없으므로, 데이터가 다 들어간 뒤 마지막에 한 번만 실행한다.

> **10~23 은 01~09 의 후속 패치다** (2026-08-16 기준 `23_admin_policy.sql` 까지 존재). 01~09 를 고치는 대신 뒤에 얹는 방식이라 실행 순서를 어기면
