...
public record ConvertNoticeToProjectRequest(
        ...
        @Schema(description = "같은 공고·회사·요청자의 COMPLETED 문서 검토 ID")
        Long reviewId,

        @Positive(message = "COMMON_INVALID_REQUEST|요약 ID가 올바르지 않습니다.")
        @Schema(description = "확정된 AI 요약 ID. 지정하면 bid_notice_summary.project_id에 연결", nullable = true)
        Long summaryId,
        ...
        @Schema(description = "추가 참여자 user ID 목록. 전환 요청자는 서버가 자동 포함", nullable = true)
