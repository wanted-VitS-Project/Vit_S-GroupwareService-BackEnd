...
public interface AccountQueryPort {
     ...
     */
    Optional<AccountTargetRow> findTarget(String userId);

    /**
     * 비밀번호 재설정 대상들의 스냅샷을 한 번에 조회한다.
     * 반환 개수가 요청 개수보다 적으면 존재하지 않는 사번이 섞인 것 → 전체 거부({@code ACC_NOT_FOUND}).
     */
    List<AccountTargetRow> findTargets(Collection<String> userIds);
}
