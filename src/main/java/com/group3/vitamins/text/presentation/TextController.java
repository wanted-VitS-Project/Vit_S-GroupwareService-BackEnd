...
public class TextController {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400",
                    description = "내용을 입력해 주세요. (TXT-003) / 버전 정보가 없습니다. (TEXT_VERSION_REQUIRED)"),
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409",
                    description = "다른 사용자가 먼저 수정했습니다. (TEXT_VERSION_CONFLICT)"),
    ...
    public ResponseEntity<ApiResponse<UpdateTextContentResponse>> updateTextContent(
        ...
        UpdateTextContentView view = textCommandUseCase.updateContent(
                new UpdateTextContentCommand(authentication.getName(), txtId, request.content(),
                        request.version(), Boolean.TRUE.equals(request.overwrite()),
                        RequesterRole.from(authentication)));
        UpdateTextContentResponse data =
                new UpdateTextContentResponse(view.txtId(), view.content(), view.updatedAt(), view.version());
