...
public interface SpringDataApprovalLineRepository extends JpaRepository<ApprovalLineJpaEntity, Long> {
    ...
    @Query("SELECT l FROM ApprovalLineJpaEntity l WHERE l.approvalLineId = :lineId")
    ...
    @Query("UPDATE ApprovalLineJpaEntity l SET l.status = :status, l.opinion = :opinion, "
            + "l.processedAt = CURRENT_TIMESTAMP WHERE l.approvalLineId = :lineId")
    ...
    Optional<ApprovalLineJpaEntity> findByApprovalRevisionIdAndSequenceNo(Long approvalRevisionId, int sequenceNo);
    ...
    @Query("UPDATE ApprovalLineJpaEntity l SET l.status = :canceled WHERE l.approvalRevisionId = :revisionId "
            + "AND l.sequenceNo > :sequenceNo AND l.status = :waiting")
