...
public record ProjectDetailResponse(
    ...
    /** 생성 결과를 응답으로 옮긴다. */
    public static ProjectDetailResponse from(ProjectResult result) {
        List<BusinessCategorySummaryResponse> categories = result.businessCategories().stream()
                .map(c -> new BusinessCategorySummaryResponse(c.categoryId(), c.name()))
                .toList();
    ...
    public record BusinessCategorySummaryResponse(
            ...
            @Schema(description = "카테고리 이름", example = "환경") String name
    ) {
    }
