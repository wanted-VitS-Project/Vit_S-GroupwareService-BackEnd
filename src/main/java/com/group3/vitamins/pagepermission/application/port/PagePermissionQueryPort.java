...
public interface PagePermissionQueryPort {
    ...
    /** §2 부여 인원 수 — 회사 사원 기준. */
    long countGrants(String pageCode, Long companyId);

    /** §2·§3 전역 권한 인원 = MASTER 사원 수(비시스템·회사). */
    long countMasters(Long companyId);

    /** §2 마지막 부여 수정일(yyyy-MM-dd). 회사 사원 기준. 부여 기록 없으면 null. */
    LocalDate findLastGrantedDate(String pageCode, Long companyId);
