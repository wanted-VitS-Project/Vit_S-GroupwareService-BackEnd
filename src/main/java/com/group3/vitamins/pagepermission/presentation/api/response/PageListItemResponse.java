...
public record PageListItemResponse(
        @Schema(description = "페이지 코드", example = "FINANCE") String pageCode,
        @Schema(description = "페이지명", example = "재무 관리") String name,
        @Schema(description = "설명", example = "재무 관리 화면 접근 권한") String description,
        @Schema(description = "접근 인원 = grantedCount + globalRoleCount", example = "5") int accessCount,
        @Schema(description = "명시적 부여 인원", example = "3") int grantedCount,
        @Schema(description = "전역 권한 열람 인원", example = "2") int globalRoleCount,
        @Schema(description = "마지막 수정일 yyyy-MM-dd", nullable = true, example = "2026-08-10") String lastModifiedAt
) {
