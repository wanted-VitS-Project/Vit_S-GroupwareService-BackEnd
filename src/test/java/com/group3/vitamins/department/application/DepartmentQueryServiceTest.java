...
class DepartmentQueryServiceTest {
    ...
    void setUp() {
        ...
        queryService = new DepartmentQueryService(departmentMapper);
    }
    ...
    void assemblesTwoLevelTreeAndRollsUpTotal() {
        ...
        when(departmentMapper.findAllWithDirectEmployeeCount()).thenReturn(List.of(
                new DepartmentTreeRow(1L, "경영지원본부", null, 0),
                new DepartmentTreeRow(2L, "기술본부", null, 1),
                new DepartmentTreeRow(4L, "인사팀", 1L, 2),
                new DepartmentTreeRow(5L, "회계팀", 1L, 2),
                new DepartmentTreeRow(6L, "개발팀", 2L, 5)
        ));

        DepartmentListResponse result = queryService.getDepartmentTree();

        // 최상위는 2개, 생성 순서(departmentId) 유지
        assertThat(result.content()).extracting(DepartmentTreeResponse::departmentId)
                .containsExactly(1L, 2L);

        DepartmentTreeResponse management = result.content().get(0);
        assertThat(management.directEmployeeCount()).isEqualTo(0);
        assertThat(management.totalEmployeeCount()).isEqualTo(4); // 0 + 2 + 2
        assertThat(management.children()).extracting(DepartmentTreeResponse::name)
                .containsExactly("인사팀", "회계팀");

        DepartmentTreeResponse tech = result.content().get(1);
    ...
    void leafTotalEqualsDirectAndChildrenEmpty() {
        when(departmentMapper.findAllWithDirectEmployeeCount()).thenReturn(List.of(
                new DepartmentTreeRow(1L, "경영지원본부", null, 0),
                new DepartmentTreeRow(4L, "인사팀", 1L, 3)
        ));

        DepartmentTreeResponse leaf = queryService.getDepartmentTree().content().get(0).children().get(0);
    ...
    void rootWithoutChildren() {
        when(departmentMapper.findAllWithDirectEmployeeCount()).thenReturn(List.of(
                new DepartmentTreeRow(3L, "감사실", null, 2)
        ));

        DepartmentTreeResponse root = queryService.getDepartmentTree().content().get(0);
    ...
    @Test
    @DisplayName("부서가 하나도 없으면 content 는 빈 배열이다")
    void emptyWhenNoDepartments() {
        when(departmentMapper.findAllWithDirectEmployeeCount()).thenReturn(List.of());

        assertThat(queryService.getDepartmentTree().content()).isEmpty();
    }
}
